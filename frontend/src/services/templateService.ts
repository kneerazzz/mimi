...
export const templateByCategory = async ( category: string, subCategory: string ) => {
    const response = await api.get(`/template/get-category-templates?category=${category}&subCategory=${subCategory}`);
    return response.data;
}
